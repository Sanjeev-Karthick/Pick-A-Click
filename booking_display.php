...
<script src="https://use.fontawesome.com/60db52c5b9.js"></script>
    ...
    <?php
    $sql = 'select * from booking where cust_id = '.$id.'';
    ...
    while($row = mysqli_fetch_assoc($result))
    {
        ...
        $date = date('d/m/Y', $s);
        $time = date('H:i A', $s);
        $std = 'select * from photostudio where stud_id = '.$studio_id.'';
        $res = mysqli_query($conn,$std);
        while($r = mysqli_fetch_assoc($res))
        {
            $studio_phone = $r["stud_phone"];
            $studio_address =$r["stud_address"];
            $studio_name = $r["stud_name"];
    ?>


<a href="#" class="list-group-item list-group-item-action flex-column align-items-start shadow p-2 mb-4  rounded">
                <div class=" justify-content-between">
                    <h5 class="mb-1 shadow-sm" style="text-transform: uppercase;"><b><i class="fa fa-user" aria-hidden="true"></i> <?php echo $studio_name;?></b>  <i class="fa fa-phone" aria-hidden="true" style="margin-left:50%"></i> : <?php echo $studio_phone;?> </h5> <hr>
                </div>
                <p class="mb-1">
